...
#define BLOCKY_CLOUDS_ALTITUDE_3 905.0  // Высота 3-го слоя
#define BLOCKY_CLOUDS_ALTITUDE_4 1600.0  // Высота 4-го слоя
#define BLOCKY_CLOUDS_LAYER_3            // Включить 3-й слой
#define BLOCKY_CLOUDS_LAYER_4            // Включить 4-й слой
...
const float blocky_clouds_altitude_l2 = BLOCKY_CLOUDS_ALTITUDE_3; // Новый 3-й слой
const float blocky_clouds_altitude_l3 = BLOCKY_CLOUDS_ALTITUDE_4; // Новый 4-й слой
...
vec4 raymarch_blocky_clouds(
    ...
    // Определяем offset для каждого слоя
    float layer_offset = 0.0;
    
#if defined(BLOCKY_CLOUDS_LAYER_2) || defined(BLOCKY_CLOUDS_LAYER_3) || defined(BLOCKY_CLOUDS_LAYER_4)
    if (abs(layer_altitude - blocky_clouds_altitude_l1) < 0.5) {
        layer_offset = 3000.0;
    } else if (abs(layer_altitude - blocky_clouds_altitude_l2) < 0.5) {
        layer_offset = 6000.0; // Новый offset для 3-го слоя
    } else if (abs(layer_altitude - blocky_clouds_altitude_l3) < 0.5) {
        layer_offset = 9000.0; // Новый offset для 4-го слоя
    }
#endif
