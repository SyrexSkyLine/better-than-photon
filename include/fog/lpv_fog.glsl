...
mat2x3 get_lpv_fog_coefficients(vec3 position_world) {
    ...
    // LPV Style configurations
        ...
        const vec3 underground_fog_extinction =
            vec3(0.725) * LPV_VL_INTENSITY_UNDERGROUND;
        const vec3 underground_fog_scattering = underground_fog_extinction * 1.95;
        ...
        const float underground_fog_fade_length = 20.0;
    #else
        // Default Style (LPV_STYLE == 2)
        const float overworld_density_scale = 8.0 * LPV_VL_INTENSITY_OVERWORLD;
        const vec3 overworld_fog_min_extinction = vec3(0.001);
        const vec3 overworld_fog_min_scattering = overworld_fog_min_extinction;
        const vec3 underground_fog_extinction =
            vec3(0.325) * LPV_VL_INTENSITY_UNDERGROUND;
        const vec3 underground_fog_scattering = underground_fog_extinction * 1.55;
        const float underground_fog_fade_length = 20.0;
    #endif
