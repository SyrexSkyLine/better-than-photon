...
#ifndef AUTO_EXPOSURE_RATE_DIM_TO_BRIGHT
    #define AUTO_EXPOSURE_RATE_DIM_TO_BRIGHT 0.4
#endif
#ifndef AUTO_EXPOSURE_RATE_BRIGHT_TO_DIM
    #define AUTO_EXPOSURE_RATE_BRIGHT_TO_DIM 1.2
#endif
...
void main() {
    ...
    if (isnan(previous_exposure) || isinf(previous_exposure) || previous_exposure <= 0.0) {
        previous_exposure = target_exposure;
    }

    // Разные скорости адаптации:
    // target < previous => сцена стала ярче (день после ночи) => адаптируемся медленно
    // target > previous => сцена стала темнее (ночь после дня) => адаптируемся быстрее
